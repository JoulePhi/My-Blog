    ...
    return (
        <>
            <div className={` ${isOpen ? "opacity-100 z-50" : "opacity-0 -z-20"} fixed duration-100 transition-opacity  w-full h-full top-0 hidden md:flex justify-center items-center bg-black/50  backdrop-blur-sm`}>
                <div className="relative bg-white dark:bg-darkBg rounded-lg shadow  w-2/3  lg:w-1/2">
                                ...
                                results.map((result, index) => (
                                    <div className="flex h-20 w-full" key={index}>
                                        ...
                                        <div className="flex flex-col w-3/4 text-ellipsis line-clamp-2">
                                            <Link href={'/post/' + result.slug} className="hover:underline font-poppins h-full w-full dark:text-textDark" onClick={() => setIsOpen(false)}>{result.title}</Link>
